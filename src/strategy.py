...
class PullbackReversalStrategy:
    def __init__(
        self,
        name,
        timeframe,
    ...
    def entry_signal(self, trader):
        ...
        # print(ta.increasing(trader.data_window.histogram, length=self.entry_window))
        if (
            # (ta.increasing(trader.data_window.histogram, length=self.entry_window).values[-1] == 1)
            (trader.data_window.histogram.values[-1] > trader.data_window.histogram.values[-2])
            and (trader.data_window.hist_ema.values[-1] > trader.data_window.hist_ema.values[-2])
            and (trader.data_window.close.values[-1] <= trader.data_window.ci.values[-1])
            # and (np.alltrue(trader.data_window.histogram.tail(self.entry_window) >= 0))
            # and (ta.increasing(trader.data_window.histogram, length=self.entry_window).values[-1] == 1)
...
#         ):
            ...
            (trader.data_window.histogram.values[-1] < trader.data_window.histogram.values[-2])
            and (trader.data_window.hist_ema.values[-1] < trader.data_window.hist_ema.values[-2])
            and (trader.data_window.close.values[-1] >= trader.data_window.cs.values[-1])
        ):
            trader.position_type = -1
            return True
        else:
...
class VolatilityStrategy:
    ...
    def entry_signal(self, trader):
        ...
        if (
            ...
            (trader.data_window.close.values[-1] >= trader.data_window.cs.values[-1])
            and trader.ta_handler.signal == -1
        ):
            trader.position_type = -1
            return True
